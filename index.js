...
/*
    ...
    ### Mock.mock( template|function() )

    根据数据模板生成模拟数据。
*/
module.exports = function(template) {
    return Handle.gen(template);
};
...
_.extend(Handle, {
    array: function(options) {
        ...
        if (!options.rule.parameters) {
            ...
            if (options.rule.min === 1 && isNaN(options.rule.max) && options.template.length > 1) {
                // fix #17
                ...
                for (i = 0; i < options.rule.count; i++) {
                    options.context.path.push(options.name);
                    result.push(Random.pick(Handle.gen(options.template, undefined, {
                        currentContext: result,
                        templateCurrentContext: options.template,
                        path: options.context.path
                    })));
        ...
        if (options.rule.min) {
            ...
            */
            ...
            for (i = 0; i < keys.length; i++) {
                ...
                // 'id|+1': 1
                inc = key.match(rkey);
                if (inc && inc[2] && _.type(options.template[key]) === 'number') {
